...
class FEALocalStiffComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        for lifting_surface_name, lifting_surface_data in lifting_surfaces:
            ...
            self.add_output(local_name, shape=(num_points_z - 1, 12, 12))
    ...
    def compute_partials(self, inputs, partials):
        ...
        for lifting_surface_name, lifting_surface_data in lifting_surfaces:
            ...
            derivs_A = partials[local_name, A_name].reshape((num_points_z - 1, 12, 12))
            derivs_Iy = partials[local_name, Iy_name].reshape((num_points_z - 1, 12, 12))
            derivs_Iz = partials[local_name, Iz_name].reshape((num_points_z - 1, 12, 12))
            derivs_J = partials[local_name, J_name].reshape((num_points_z - 1, 12, 12))
            derivs_L = partials[local_name, L_name].reshape((num_points_z - 1, 12, 12))
