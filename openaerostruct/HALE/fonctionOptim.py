...
#    materials=[al7075, fakemat, steel, nomat]
    ...
    prob.model.add_subsystem('PointMassLocations', PointMassLocations(surface=surface), promotes_inputs=['engine_location', 'span', 'nodes'], promotes_outputs=['point_mass_locations'])  #VMGM        
    prob.model.add_subsystem('PointMasses', PointMasses(surface=surface), promotes_inputs=['PV_surface'], promotes_outputs=['point_masses'])  #VMGM
    
    prob.model.add_subsystem('W0_comp',
        ExecComp('W0 = W0_without_point_masses + 2*sum(point_masses)', units='kg'),
        promotes=['*'])
...
#    for i in range(1):
    ...
    print("tbs1",prob['AS_point_1.wing_perf.struct_funcs.vonmises.top_bending_stress'])
    print("tbs2",prob['AS_point_2.wing_perf.struct_funcs.vonmises.top_bending_stress'])
    print("disp0",prob['AS_point_0.coupled.wing.struct_states.disp.disp'])
    print("disp1",prob['AS_point_1.coupled.wing.struct_states.disp.disp'])
    print("disp2",prob['AS_point_2.coupled.wing.struct_states.disp.disp'])
