...
class SectionPropertiesWingbox(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        for j in range(self.ny - 1):
            ...
            chord = inputs['fem_chords'][j]
            spar_thickness = inputs['spar_thickness'][j]
            skin_thickness = inputs['skin_thickness'][j]
            t_over_c_original = self.orig_wb_af_t_over_c
            t_over_c = inputs['t_over_c'][j]
            streamwise_chord = inputs['streamwise_chords'][j]
            theta = inputs['fem_twists'][j]

            # Scale data points with chord
            data_x_upper = chord * self.data_x_upper
            data_y_upper = chord * self.data_y_upper
            data_x_lower = chord * self.data_x_lower
            data_y_lower = chord * self.data_y_lower
            ...
            outputs['A_enc'][j] = A_enc
            outputs['A_int'][j] = A_int
            ...
            p_by_t = np.sum(p_by_t_1 + p_by_t_2)
            ...
            area = np.sum(upper_area) + np.sum(lower_area) + area_spars

            outputs['A'][j] = area

            centroid = (np.sum(first_moment_area_upper) + np.sum(first_moment_area_lower) + first_moment_area_front_spar + first_moment_area_rear_spar) / area
            ...
            I_horiz = np.sum(I_horiz_1 + I_horiz_2)
            ...
            I_horiz += np.sum(2 * ((1./12. * a**3 * x2**4 + 1./3. * a**2 * x2**3 * b + 1./2. * a * x2**2 * b**2 + 1./3. * b**3 * x2)))
            I_horiz += np.sum(x2 * skin_thickness * ((-y_low_add)/2 - skin_thickness/2 + centroid)**2)
            ...
            outputs['Iz'][j] = I_horiz

            # Compute the Q required for transverse shear stress due to upward bending
            Q_upper = np.sum(((y_up_add / 2 - (skin_thickness/2) ) - centroid) * skin_thickness * x_up_add)

            Q_upper += (data_y_upper[0] - skin_thickness - centroid)**2 / 2 * (spar_thickness)
            Q_upper += (data_y_upper[-1] - skin_thickness - centroid)**2 / 2 * (spar_thickness)

            outputs['Qz'][j] = Q_upper
            ...
            outputs['htop'][j] = htop
            outputs['hbottom'][j] = hbottom
            outputs['hfront'][j] = hfront
            outputs['hrear'][j] = hrear
