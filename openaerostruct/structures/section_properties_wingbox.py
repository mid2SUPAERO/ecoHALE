...
class SectionPropertiesWingbox(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('chords_fem', val=np.ones((self.ny - 1), dtype = complex))
        self.add_input('streamwise_chords', val=np.ones((self.ny - 1), dtype = complex))
        self.add_input('twist_fem', val=np.ones((self.ny - 1),  dtype = complex))

        self.add_input('sparthickness', val=np.ones((self.ny - 1), dtype = complex))
        self.add_input('skinthickness', val=np.ones((self.ny - 1),  dtype = complex))
    ...
    def compute(self, inputs, outputs):
        ...
        for i in range(self.ny - 1):
            ...
            wingbox_props(inputs['chords_fem'][i], inputs['sparthickness'][i], inputs['skinthickness'][i], self.data_x_upper, \
            self.data_x_lower, self.data_y_upper, self.data_y_lower, self.t_over_c, inputs['toverc'][i], inputs['streamwise_chords'][i], -inputs['twist_fem'][i])
