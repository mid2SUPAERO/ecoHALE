...
&              ind)-quarter_chord(iy, ind))*meshb(ix, iy, ind)
            meshb(ix, iy, ind) = taper_lins(ny-iy+1)*meshb(ix, iy, ind)
          end do
        end do
      end do
...
&              quarter_chord(iy, ind))*meshb(ix, iy, ind)
            meshb(ix, iy, ind) = dx(ny-iy+1)*meshb(ix, iy, ind)
          end do
        end do
      end do
...
&    3), t4b(3)
    ...
    do ielem=n-1,1,-1
      ...
      temp0b = .5*(sxx1**2+sxt**2)**(-0.5)*vonmisesb(ielem, 2)
      sxx1b = 2*sxx1*temp0b
      vonmisesb(ielem, 2) = 0.0_8
      sxx0 = e*(u1(1)-u0(1))/l + e*r2(ielem)/l*tmp
      temp0b0 = .5*(sxx0**2+sxt**2)**(-0.5)*vonmisesb(ielem, 1)
      sxtb = 2*sxt*temp0b0 + 2*sxt*temp0b
      sxx0b = 2*sxx0*temp0b0
      vonmisesb(ielem, 1) = 0.0_8
      r1r0b = 0.0_8
      temp = r1r0(1)/l
      tempb = r2(ielem)*g*sxtb/l
      r2b(ielem) = r2b(ielem) + e*tmp*sxx1b/l + e*tmp*sxx0b/l + g*temp*&
&        sxtb
      r1r0b(1) = r1r0b(1) + tempb
      u0b = 0.0_8
      u1b = 0.0_8
      tempb0 = e*sxx1b/l
      tempb1 = r2(ielem)*e*sxx1b/l
      tempb3 = e*sxx0b/l
      u0b(1) = u0b(1) + tempb0 - tempb3
      u1b(1) = u1b(1) + tempb3 - tempb0
      tempb2 = r2(ielem)*e*sxx0b/l
      lb = -((u0(1)-u1(1))*tempb0/l) - tmp*tempb1/l - tmp*tempb2/l - (u1&
&        (1)-u0(1))*tempb3/l - temp*tempb
      tmpb = tempb2 + tempb1
      tempb4 = .5*(r1r0(2)**2+r1r0(3)**2)**(-0.5)*tmpb
      r1r0b(2) = r1r0b(2) + 2*r1r0(2)*tempb4
      r1r0b(3) = r1r0b(3) + 2*r1r0(3)*tempb4
      r0b = 0.0_8
      r1b = 0.0_8
...
&               , mat12x2b)
      ...
      temp0 = l**3
      temp0b = e*eiz_l3b/temp0
      izb(ielem) = izb(ielem) + temp0b
      temp = l**3
      tempb = e*eiy_l3b/temp
      iyb(ielem) = iyb(ielem) + tempb
      tempb1 = g*gj_lb/l
      jb(ielem) = jb(ielem) + tempb1
      tempb0 = e*ea_lb/l
      lb = lb - iy(ielem)*3*l**2*tempb/temp - a(ielem)*tempb0/l - j(&
&        ielem)*tempb1/l - iz(ielem)*3*l**2*temp0b/temp0
      ab(ielem) = ab(ielem) + tempb0
      p0 = nodes(ielem, :)
      p1 = nodes(ielem+1, :)
      arg1(:) = p1 - p0
      call popreal8(l)
